...
class RopeActionDataset(RopeDataset):
    ...
    def __getitem__(self, idx):
        ...
        batch_i = idx // (2 * self.num_batches)
        sample_i = (idx % (2 * self.batch_size)) // 2
        acts = np.load(f"{self.data_path}/batch_{batch_i}/action_hats.npy",allow_pickle=True)
        p_label = acts[sample_i]['rope_site']
        th_label = acts[sample_i]['angle']
        len_label = acts[sample_i]['length']

        p_label = torch.tensor(self.discretize(p_label, self.p_range, self.p_dim)).long()
        p_label = F.one_hot(p_label, self.p_dim)

        th_label = torch.tensor(self.discretize(th_label, self.th_range, self.th_dim)).long()
        th_label = F.one_hot(th_label, self.th_dim)

        len_label = torch.tensor(self.discretize(len_label, self.len_range, self.len_dim)).long()
